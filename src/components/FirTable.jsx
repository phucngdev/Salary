...
const FirTable = () => {
  const [time, setTime] = useState(() => {
    const timeLocal = JSON.parse(localStorage.getItem(`firMonth`)) || [];
    return timeLocal;
  });
  ...
  const handleOk = () => {
    ...
    localStorage.setItem(`firMonth`, JSON.stringify(time));
    message.success({
  ...
  const showDeleteConfirm = () => {
    confirm({
      ...
      cancelText: "No",
      onOk() {
        localStorage.removeItem("firMonth");
        message.success({
  ...
  const handleDelete = (id) => {
    confirm({
      ...
      onOk() {
        ...
        localStorage.setItem(`firMonth`, JSON.stringify(itemDelete));
